...
let baseURL = `https://690829bfb49bea95fbf28bda.mockapi.io/`;
...
async function fetchAllDestinations(page) {
  try {
    ...
    let url = `${destinationURL}?page=${page}&limit=${limit}`;
...
//     paginationContainer.innerHTML = "";
//     loader.style.display = "block";
//     let url = `${destinationURL}?q=${searchinp.value}&page=${page}&limit=${limit}`;
//     let res = await fetch(url);
//     totalCount = res.headers.get("X-Total-Count");
//     numPages = Math.ceil(totalCount / limit);
//     let data = await res.json();
...
//   }
// }
async function fetchBySearch(page = 1) {
  try {
    main_container.innerHTML = "";
    paginationContainer.innerHTML = "";
    loader.style.display = "block";
    ...
    // Filter manually (MockAPI doesn't support ?q=)
    ...
    // Display results
    displayDestinations(paginatedData);

    // Pagination buttons
    createPaginationBtns(totalPages, page);

  } catch (err) {
    console.error("Error fetching by search:", err);
  } finally {
    loader.style.display = "none";
  }
}
...
//   let url;
//   if (str == "asc") {
//     url = `${destinationURL}?sort=estimatedCost&order=asc&page=${page}&limit=12`;
//   } else if (str == "desc") {
//     url = `${destinationURL}?sort=estimatedCost&order=desc&page=${page}&limit=12`;
//   }
//   try {
//     let res = await fetch(url);
//     totalCount = res.headers.get("X-Total-Count");
...
async function filterPriceSort(str, page = 1) {
  try {
    loader.style.display = "block";
    main_container.innerHTML = "";
    paginationContainer.innerHTML = "";
    ...
    // Display sorted & paginated data
    displayDestinations(paginatedData);
    createFilterPaginationBtns(totalPages, str, page);
  } catch (err) {
...
async function filterDestinationsByCountry(page) {
  ...
  if (value == "") {
    ...
    try {
      let url = `${destinationURL}?country=${value}&page=${page}&limit=${limit}`;
      let res = await fetch(url);
      totalCount = res.headers.get("X-Total-Count");
      numPages = Math.ceil(totalCount / limit);
      let data = await res.json();
...
//   let url;
//   if (num == 5) {
//     url = `${destinationURL}?rating_gte=4&page=${page}&limit=${limit}`;
//   } else if (num == 4) {
//     url = `${destinationURL}?rating_gte=3&rating_lte=3.9&page=${page}&limit=${limit}`;
//   } else if (num == 3) {
//     url = `${destinationURL}?rating_gte=2&rating_lte=2.9&page=${page}&limit=${limit}`;
//   } else if (num == 2) {
//     url = `${destinationURL}?rating_gte=1&rating_lte=1.9&page=${page}&limit=${limit}`;
//   }
//   try {
//     let res = await fetch(url);
//     totalCount = res.headers.get("X-Total-Count");
...
async function filterDestinationsByRating(num, page = 1) {
  try {
    loader.style.display = "block";
    main_container.innerHTML = "";
    paginationContainer.innerHTML = "";
    ...
    // Filter manually based on rating
    let filteredData = allData.filter((item) => {
      const rating = Number(item.rating);
      if (num === 5) return rating >= 4;
      if (num === 4) return rating >= 3 && rating < 4;
      if (num === 3) return rating >= 2 && rating < 3;
      if (num === 2) return rating >= 1 && rating < 2;
    });
    ...
    displayDestinations(paginatedData);
    createPaginationBtnsByRating(num, totalPages, page);
  } catch (err) {
    console.error("Error filtering by rating:", err);
  } finally {
    loader.style.display = "none";
  }
}
...
//   let url;
//   if (str == "above 7500") {
//     url = `${destinationURL}?estimatedCost_gte=7500&page=${page}&limit=${limit}`;
//   } else if (str == "6000-7500") {
//     url = `${destinationURL}?estimatedCost_gte=6000&estimatedCost_lte=7499&page=${page}&limit=${limit}`;
//   } else if (str == "4000-6000") {
//     url = `${destinationURL}?estimatedCost_gte=4000&estimatedCost_lte=5999&page=${page}&limit=${limit}`;
//   } else if (str == "below 4000") {
//     url = `${destinationURL}?estimatedCost_lte=3999&_page=${page}&limit=${limit}`;
//   }
//   try {
//     let res = await fetch(url);
//     totalCount = res.headers.get("X-Total-Count");
...
//   }
// }
async function filterDestinationsByPrice(str, page = 1) {
  try {
    loader.style.display = "block";
    main_container.innerHTML = "";
    paginationContainer.innerHTML = "";
    ...
    // Filter manually in JavaScript (MockAPI doesn't support _gte/_lte)
    let filteredData = allData.filter((item) => {
      const cost = Number(item.estimatedCost);
      if (str === "above 7500") return cost >= 7500;
      if (str === "6000-7500") return cost >= 6000 && cost < 7500;
      if (str === "4000-6000") return cost >= 4000 && cost < 6000;
      if (str === "below 4000") return cost < 4000;
    });
    ...
    displayDestinations(paginatedData);
    createPaginationBtnsByPrice(str, totalPages, page);
  } catch (err) {
    console.error("Error filtering destinations by price:", err);
  } finally {
    loader.style.display = "none";
  }
}
...
async function getUser(id) {
  let url = `https://690829bfb49bea95fbf28bda.mockapi.io/users?id=${id}`;
  try {
    let res = await fetch(url);
    return res.json();
  } catch (e) {
...
async function postCart(id, userCart) {
  const url = `https://690829bfb49bea95fbf28bda.mockapi.io/users/${id}`;
  try {
    ...
    displayMessage("Failed to update cart", "warning");
  }
}
